...
class LandNavView extends Ui.DataField {
	...
	function onUpdate(dc) 
    {
		...
		dc.drawLine( 0, 101, 240, 101); // horizontal
	    dc.drawLine( 110, 101, 110, 0 ); // vertical 

		if (nP > 0) { 
			if (App.getApp().getProperty("M")) {  				// M G R S 
			    dc.drawText( 208, 112, 4, cr.substring(0,3) + cr.substring(4,12), 0 );   
			    dc.drawText( 208, 142, 4, cr.substring(12,17), 0 ); 
			} else {															// L A T & L O N 
			    var sLb = cr.find("W");
			    if (sLb == null) {
					sLb = cr.find("E");
				...
				if (sLb == null) {
			   	...
			   	dc.drawText( 125, 112, 4, cr.substring(0,sLb), 1 );
			   	dc.drawText( 125, 142, 4, cr.substring(sLb, cr.length()), 1 );
			}
			if (App.getApp().getProperty("Z")) { 
				dc.drawText( 125, 46, 6, sDs, 2 ); 							// distance
	
		    	dc.setColor( Gfx.COLOR_RED, -1 );  								// direction
				dc.drawText( 94, 46, 6, sDr, 0 ); 
			}
		} else { // if (nP <= 0) 
		    dc.drawText( 208, 112, 4, "Points Error", 0 );   
		} 
	}
//EOC---------------------------------------------------------------------------------------------
}	    			
