...
class LandNavView extends Ui.DataField {
	...
	function onUpdate(dc) 
    {
		...
		dc.drawLine( 0, 103, 218, 103); // horizontal
	    dc.drawLine( 99, 103, 99, 0  ); // vertical 
		
		if (nP > 0) { 
			if (App.getApp().getProperty("M")) {  				// M G R S 
			    dc.drawText( 188, 114, 4, cr.substring(0,3) + cr.substring(4,12), 0 );   
			    dc.drawText( 188, 144, 4, cr.substring(12,17), 0 ); 
			} else {															// L A T & L O N 
			    var sLb = cr.find("W");
			    if (sLb == null) {
					sLb = cr.find("E");
				...
				if (sLb == null) {
			   	...
			   	dc.drawText( 109, 109, 4, cr.substring(0,sLb), 1 );
			   	dc.drawText( 109, 139, 4, cr.substring(sLb, cr.length()), 1 );
			}
			if (App.getApp().getProperty("Z")) { 
				dc.drawText( 115, 52, 6, sDs, 2 ); 							// distance
	
		    	dc.setColor( Gfx.COLOR_RED, -1 );  								// direction
				dc.drawText( 82, 52, 6, sDr, 0 ); 
			}
		} else { // if (nP <= 0) 
		    dc.drawText( 188, 114, 4, "Points Error", 0 );   
		} 
	}
//EOC---------------------------------------------------------------------------------------------
}	    
