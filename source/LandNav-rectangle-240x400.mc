...
class LandNavView extends Ui.DataField {
	...
	function onUpdate(dc) 
    {
		...
		dc.drawLine( 0, 55, 240, 55); // horizontal
	    dc.drawLine( 110, 55, 110, 0 ); // vertical 

		if (nP > 0) { 
			if (App.getApp().getProperty("M")) {  				// M G R S 
			    dc.drawText( 203, 61, 4, cr.substring(0,3) + cr.substring(4,12), 0 );   
			    dc.drawText( 203, 91, 4, cr.substring(12,17), 0 ); 
			} else {															// L A T & L O N 
			    var sLb = cr.find("W");
			    if (sLb == null) {
					sLb = cr.find("E");
				...
				if (sLb == null) {
			   	...
			   	dc.drawText( 124, 61, 4, cr.substring(0,sLb), 1 );
			   	dc.drawText( 124, 91, 4, cr.substring(sLb, cr.length()), 1 );
			}
			if (App.getApp().getProperty("Z")) { 
				dc.drawText( 122, 10, 6, sDs, 2 ); 							// distance
	
		    	dc.setColor( Gfx.COLOR_RED, -1 );  								// direction
				dc.drawText( 97, 10, 6, sDr, 0 ); 
			}
		} else { // if (nP <= 0) 
		    dc.drawText( 203, 61, 4, "Points Error", 0 );   
		} 
	}
//EOC---------------------------------------------------------------------------------------------
}
