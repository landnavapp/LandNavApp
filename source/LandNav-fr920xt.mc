...
class LandNavView extends Ui.DataField {
	...
	function onUpdate(dc) 
    {
    	...
    	//////App.getApp().setProperty("V", "1.70331b " + (z/10) + "/" + ((450 - (25*nP))/10).toNumber() + ", " + ((Sys.getSystemStats().freeMemory.toFloat()/Sys.getSystemStats().totalMemory.toFloat()) * 100).toNumber() + "%");
	    dc.setPenWidth(3); //=========== THICK lines============================================================
		dc.drawLine( 0, 58, 205, 58); // horizontal
	    dc.drawLine( 93, 58, 93, 0  ); // vertical 

		if (nP > 0) { 		
			if (App.getApp().getProperty("M")) {  				// M G R S 
			    dc.drawText( 178, 74, 4, cr.substring(0,3) + cr.substring(4,12), 0 );   
			    dc.drawText( 178, 107, 4, cr.substring(12,17), 0 ); 
			} else {															// L A T & L O N 
			    var sLb = cr.find("W");
			    if (sLb == null) {
					sLb = cr.find("E");
				...
				//System.println("cr = " + cr);   // fr920xt has a problem displaying this, println works fine...
			   	dc.drawText( 103, 70, 6, cr.substring(0,sLb), 1 );
			   	dc.drawText( 103, 108, 6, cr.substring(sLb, cr.length()), 1 );
			}
			if (App.getApp().getProperty("Z")) { 
				dc.drawText( 108, 13, 6, sDs, 2 ); 							// distance
	
		    	dc.setColor( Gfx.COLOR_RED, -1 );  								// direction
				dc.drawText( 77, 13, 6, sDr, 0 ); 
			}
		} else { // if (nP <= 0) 
		    dc.drawText( 178, 74, 4, "Points Error", 0 );   
		} 
	}
//EOC---------------------------------------------------------------------------------------------
}
