...
//////////////////Attn.playTone(15);				
		...
		dc.drawLine( 0, 68, 205, 68); // horizontal
	    dc.drawLine( 93, 68, 93, 0  ); // vertical 

		if (nP > 0) { 
			if (App.getApp().getProperty("M")) {  				// M G R S 
			    dc.drawText( 174, 77, 4, cr.substring(0,3) + cr.substring(4,12), 0 );   
			    dc.drawText( 174, 107, 4, cr.substring(12,17), 0 ); 
			} else {															// L A T & L O N 
			    var sLb = cr.find("W");
			    if (sLb == null) {
					sLb = cr.find("E");
				...
				if (sLb == null) {
			   	...
			   	dc.drawText( 107, 77, 4, cr.substring(0,sLb), 1 );
			   	dc.drawText( 107, 107, 4, cr.substring(sLb, cr.length()), 1 );
			}
			if (App.getApp().getProperty("Z")) { 
				dc.drawText( 108, 19, 6, sDs, 2 ); 							// distance
	
		    	dc.setColor( Gfx.COLOR_RED, -1 );  								// direction
				dc.drawText( 77, 19, 6, sDr, 0 ); 
			}
		} else { // if (nP <= 0) 
		    dc.drawText( 174, 77, 4, "Points Error", 0 );   
		} 
	}
//EOC---------------------------------------------------------------------------------------------
}
